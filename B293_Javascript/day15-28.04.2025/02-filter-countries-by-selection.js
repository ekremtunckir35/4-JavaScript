// 1- Data'yı al:
import {countries}from "../day14-25.04.2025/data/countries.js";
...
countrySelectEl.addEventListener("change", (event) => {
    ...
    const selectedCountry = countries.filter((country) => country?.ccn3 === event.target.value);
...
function populateSelect(array){
    ...
    // Burada kaldık:
    const optionElements = array.map((item) => `
    <option value=${item?.ccn3} >${item?.name?.common || item?.name?.official }</option>
    `).join("");
